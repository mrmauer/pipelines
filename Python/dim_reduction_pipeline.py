...
class ToTensor(object):
    def __call__(self, sample):
        m = torch.from_numpy(sample).type(torch.float).reshape((28,28, -1))
        return m

# the autoencoder network
class ConvVAE(nn.Module):
    def __init__(self, latent_dim=20, init_channels=8, kernel_size=4):
...
class DenoiseAE(object):
    ...
    """
    def __init__(self, n_components, lr=1.0e-3, batch_size=64, 
                 cuda=True, path="vae.pth", kkl=1.0, kv=1.0):
        self.model = ConvVAE(latent_dim=n_components)
...
def conv_vae_emb(train_subset, dev_subset, train_data, dev_data, test_data, 
